    ...
    class Service
      class << self
        def generate_dsl(form)
          ...
          events_list = form.events.reject(&:blank?).map { |e| ":#{e}" }.join(", ")
          environments_list = form.environments.reject(&:blank?).map { |e| ":#{e}" }.join(", ")
          
          # Format function name (can be string or symbol)
          function_ref = form.function_name.to_s.match?(/\A[a-z0-9_]+\z/) ? ":#{form.function_name}" : "\"#{form.function_name}\""
          ...
          if form.environments.reject(&:blank?).any?
        ...
        def generate_migration(form)
          ...
          events_sql = form.events.reject(&:blank?).map(&:upcase).join(" OR ")
          ...
          code = <<~RUBY
            ...
            # Generated by pg_triggers on #{Time.current.strftime("%Y-%m-%d %H:%M:%S")}
            class #{class_name} < PgTriggers::Migration
              def up
                # Create the function
                execute <<-SQL
                  #{function_body_sql}
        ...
        def create_trigger(form, actor:)
          ...
          attributes = {
            ...
            enabled: form.enabled,
            source: "dsl",
            environment: form.environments.reject(&:blank?).join(",").presence,
          ...
          if TriggerRegistry.column_names.include?('condition')
            attributes[:condition] = form.condition.presence
          end
        ...
        def next_migration_number
          ...
          if Dir.exist?(triggers_path)
            existing = Dir.glob(triggers_path.join("*.rb"))
