...
module PermissionHelpers
  ...
  def with_permission_checker(permission_map = {})
    ...
    PgSqlTriggers.permission_checker = lambda do |actor, action, environment|
  ...
  # @return [void]
  def with_all_permissions_allowed
    with_permission_checker({}) do
      yield
    end
  end
  ...
  # @return [void]
  def with_permission_denied(action)
    with_permission_checker({ action => false }) do
      yield
    end
  end
  ...
  def with_kill_switch_disabled
    with_kill_switch(enabled: false) do
      yield
    end
  end
  ...
  # @yield Block to execute with the kill switch configuration
  # @return [void]
  def with_kill_switch_protecting(environment, confirmation_required: true)
    with_kill_switch(
      enabled: true,
